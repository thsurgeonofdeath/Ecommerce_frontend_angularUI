...
export class CaddyService {
  ...
  public loadCaddyFromLocalStorage(){
       let myCaddiesList=localStorage.getItem("ListCaddies_"+this.authService.userAutenticated.username);
       this.listCaddies=myCaddiesList==undefined?[{num:1,name:'Caddy1'}]:JSON.parse(myCaddiesList);
       this.listCaddies.forEach(c=>{
         let cad=localStorage.getItem("myCaddy_"+this.authService.userAutenticated.username+"_"+c.name);
  ...
  saveCaddy() {
    let caddy=this.caddies[this.currentCaddyName];
    localStorage.setItem("myCaddy_"+this.authService.userAutenticated.username+"_"+this.currentCaddyName,JSON.stringify(caddy));
  }
  ...
  addNewCaddy(c: { num: number; name: string }) {
    ...
    localStorage.setItem("ListCaddies_"+this.authService.userAutenticated.username,JSON.stringify(this.listCaddies));
  }
